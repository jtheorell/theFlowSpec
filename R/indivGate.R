#' @export indivGate
indivGate <- function(inDatDonList, marker, lowPeak=TRUE, madVals=seq(2.5, 3.5, by=0.1), turnOff=FALSE, adjust=2, volRatio=0.05, createDir=TRUE){
  if(createDir==TRUE){
    oldDir <- getwd()
  ...
  for(i in 1:length(inDatDonList)){
    markerFocus <- inDatDonList[[i]][,marker]
    ...
    #Now, it is decided which of the MAD values that should be used, depending on which value that givesrise tot he most separation.
    #gateVal <- peakValue+mad(optimPeak)*madVal

    if(nrow(inDatDonList[[i]])>10000){
      inDataSample <- inDatDonList[[i]][sample(1:nrow(inDatDonList[[i]]), 10000),]
    } else {
      inDataSample <- inDatDonList[[i]]
    }
    dataScaled <- dScale(inDataSample)

    robustSdMark <- sd(dScale(inDataSample[,marker], center=FALSE, scale=FALSE, truncate=TRUE))
    peakScaled <- peakValue/robustSdMark
    optimPeakMadScaled <- mad(optimPeak)/robustSdMark
    allCentDiff <- sapply(madVals, function(x) madDecision(dataScaled, marker, peakScaled+(optimPeakMadScaled*x)))
    madVal <- madVals[which.max(allCentDiff)[1]]
    gateVal <- peakValue+mad(optimPeak)*madVal
    print(madVals[which.max(allCentDiff)[1]])
   #Now, it is determined if the mad data should be used, or if indeed a density-based gate is more useful
    if(twoPeaks==TRUE && turnOff==FALSE){
      if(lowPeak==TRUE && madVal>0 && length(which(Da$x[Turns]>peakValue & Da$x[Turns]<gateVal))>0){
        ...
        gateTurns <- Turns[which(Da$x[Turns]>peakValue & Da$x[Turns]<gateVal)]
        gateVal <- Da$x[gateTurns[which.min(Da$y[gateTurns])]]
        graphName <- paste0("Donor_", names(inDatDonList)[i], "_", marker, "_turnGate.pdf")
        ...
        graphName <- paste0("Donor_", names(inDatDonList)[i], "_", marker, "_madGate.pdf")
      }
